...
**Status (2026-09-02):** Decisions 1–6 locked. Heightfield dropped
(decision 7). Ring ships single-sided like Plane (open Q2 deferred).
M2 before M3. **M1 done** — factory rows + `flat_shade` / sweep /
`sides` / `arc` (offline smoke in `scripts/check-primitives-3d.mts`).
M2 next is Backdrop + Rock only.
...
Base facts (verified 2026-08-19, factory grown 2026-09-02 for M1): mesh
primitives ride `makePrimitiveNode` (src/nodes/three/primitives.ts) — a
new one costs `sizeParams` + `buildGeometry` + `geomSig` + a registry
line.
...
7. **Heightfield is out.** Not needed; skip the node. (The original
   CPU-readback design is parked with the rest of M5 if it ever comes
   back.) The 2D→3D image bridge stays on Displace / Texture Projection.
...
- **Heightfield** (`heightfield-3d`) — **dropped 2026-09-02.**
...
- M2: Backdrop profile continuity (fillet tangency); Rock — same seed
  ⇒ same geometry, volume within sane bounds of the base icosphere.
...
In-browser passes: flat-shade + toon + lineart combo look; Backdrop with
...
1. Platonics as one node with a shape enum — **yes** (decision 1).
2. Ring double-sided — **defer.** Ring ships single-sided like Plane.
   A shared `double_sided` param would touch MaterialDesc (side field).
3. Heightfield budget — **n/a**, node dropped.
4. M2 before M3 — **yes.** Scene builders first; M2 is Backdrop + Rock
   (no Heightfield).
