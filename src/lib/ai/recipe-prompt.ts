...
- Only set params marked settable in the catalog (scalar/vec/color/boolean/enum/string/color_ramp/float_curve). Respect each param's range and enum options. color_ramp takes [{position, color, alpha?}]; float_curve takes [{x, y}] (0..1, at least 2 points). Params tagged ~(not remotely settable) exist on the node but cannot be written. Never set media/structured params — with one exception: a Merge node's \`layers\` accepts [{mode, opacity}, …] to size its stack.
      ...
      ops: {
        ...
        items: {
          ...
          description:
            ...
            "rename_node (node, name) · duplicate_node (node, id, params?, name?, patch?). Nested {set_param: {...}} is rejected.",
          required: ["op"],
          properties: {
            op: {
              type: "string",
...
- To surface a param as a knob on the group, use \`expose_param\` (give a short \`label\`); \`unexpose_param\` removes it. Only params with a socket type (scalar/vec/color/boolean) can be exposed. Expression \`ch()\` channels are exposable the same way (\`expose_param\` with the channel name), or by \`add_edge\` from \`interface.inputNodeId:aux:<name>\` to \`<node>:in:<channel>\` — that mints the group input. Do not leave an edge on \`aux:__virtual__\`. The group shell seeds from the interior value (a Constant at 0.3 stays 0.3, not 0). Removing a node drops interface sockets that only fed it.
- Duplicate a node or nested group with \`duplicate_node\` (fresh local \`id\`); \`params\` patch the clone shell and \`patch: [{node, param, value}]\` retunes interior nodes by their ORIGINAL ids. Prefer this over re-inserting N near-copies.
