...
- Only set params marked settable in the catalog (scalar/vec/color/boolean/enum/string/color_ramp). Respect each param's range and enum options. Params tagged ~(not remotely settable) exist on the node but cannot be written. Never set media/structured params — with one exception: a Merge node's \`layers\` accepts [{mode, opacity}, …] to size its stack.
