// Patched ESM loader for @visioncortex/vtracer's wasm-bindgen nodejs build.
// The stock glue uses fs.readFileSync; we fetch /wasm/vtracer/vtracer_wasm_bg.wasm
// (or initSync from bytes in Node check scripts) instead.
/* @ts-self-types="./vtracer.d.ts" */
...
export default async function __wbg_init(module_or_path) {
    ...
    const bytes = module_or_path instanceof Promise || (typeof Response === "function" && module_or_path instanceof Response)
        ? await (await module_or_path).arrayBuffer()
        : module_or_path;
